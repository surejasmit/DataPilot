...
const cleaningOptions = [
  { id: 'remove', label: 'Remove affected rows', desc: 'Deletes 84 rows with missing Salary values', risk: 'Loses 0.7% of data' },
  { id: 'mean', label: 'Fill with Mean', desc: 'Mean = $67,420. Sensitive to outliers.', risk: 'Biased by extreme values' },
  { id: 'median', label: 'Fill with Median', desc: 'Median = $62,000. Robust to outliers.', risk: 'Recommended for salary data', recommended: true },
  { id: 'mode', label: 'Fill with Mode', desc: 'Mode = $55,000. Most frequent value.', risk: 'May create artificial peaks' },
  { id: 'custom', label: 'Custom value', desc: 'Enter a specific salary to fill all missing', risk: 'Requires domain knowledge' },
            ...
            <div className="bg-bg-1 border border-border-1 rounded-xl p-6">
              <div className="flex items-start gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-sm text-fg-1 mt-0.5">Column: <code className="font-mono text-fg-0">Salary</code> · 84 missing (0.7% of 12,480 rows)</p>
                </div>
              </div>
              ...
              <div className="bg-bg-2 rounded-lg p-4 mb-4">
                <div className="grid grid-cols-3 gap-4 text-center">
                  <div className="p-3 bg-bg-1 rounded">
                    <div className="text-2xl font-bold font-mono text-fg-0">84</div>
                    <div className="text-xs text-fg-2">Missing</div>
                  </div>
                  <div className="p-3 bg-bg-1 rounded">
                    <div className="text-2xl font-bold font-mono text-fg-0">$62K</div>
                    <div className="text-xs text-fg-2">Median</div>
                  </div>
                  <div className="p-3 bg-bg-1 rounded">
                    <div className="text-2xl font-bold font-mono text-fg-0">$67K</div>
                    <div className="text-xs text-fg-2">Mean</div>
                  </div>
                </div>
              </div>

              <RadioGroup value={selectedOption} onValueChange={setSelectedOption} className="space-y-2" aria-label="Cleaning options for missing Salary values">
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-fg-1">Dataset version 2 created · 84 rows imputed with median ($62,000) · <a href="#" className="text-accent hover:underline">View diff</a></p>
                  </div>
                </div>
              </motion.div>
            )}
          ...
          <div className={cn('relative', isVisible && !prefersReduced ? 'animate-fade-in-right' : '')}>
            <div className="bg-bg-0/50 backdrop-blur-sm border border-border-1 rounded-2xl overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-3 border-b border-border-1 bg-bg-1">
                ...
                <div className="ml-4 text-xs text-fg-2 font-mono text-center flex-1">employee_data.csv — Salary column preview</div>
              </div>
                  ...
                  <table className="w-full text-sm font-mono" role="table">
                    <thead>
                      <tr className="border-b border-border-1 sticky top-0 bg-bg-0">
                        ...
                        <th className="px-3 py-2 text-left text-fg-2 font-medium">Employee</th>
                        <th className="px-3 py-2 text-left text-fg-2 font-medium">Department</th>
                        <th className="px-3 py-2 text-left text-fg-2 font-medium">Salary</th>
                        <th className="px-3 py-2 text-left text-fg-2 font-medium">Status</th>
                      </tr>
                    </thead>
                    <tbody>
                      {[
                        { row: 1, employee: 'Sarah Johnson', department: 'Engineering', salary: 89500, status: 'ok' },
                        { row: 2, employee: 'Mike Chen', department: 'Sales', salary: null, status: 'missing' },
                        { row: 3, employee: 'Emily Rodriguez', department: 'Marketing', salary: 67200, status: 'ok' },
                        { row: 4, employee: 'James Wilson', department: 'HR', salary: null, status: 'missing' },
                        { row: 5, employee: 'Priya Patel', department: 'Finance', salary: 78400, status: 'ok' },
                        { row: 6, employee: 'David Kim', department: 'Engineering', salary: 92100, status: 'ok' },
                        { row: 7, employee: 'Lisa Thompson', department: 'Sales', salary: null, status: 'missing' },
                        { row: 8, employee: 'Alex Rivera', department: 'Marketing', salary: 54300, status: 'ok' },
                        { row: 9, employee: 'Jordan Lee', department: 'Finance', salary: null, status: 'missing' },
                        { row: 10, employee: 'Maria Garcia', department: 'Engineering', salary: 95400, status: 'ok' },
                      ].map((item) => (
                        <tr key={item.row} className="border-b border-border-1/50 hover:bg-bg-1 transition-colors">
                          ...
                          <td className="px-3 py-2 text-fg-1">{item.employee}</td>
                          <td className="px-3 py-2 text-fg-1">{item.department}</td>
                          <td className={cn('px-3 py-2', item.status === 'missing' ? 'text-accent' : 'text-fg-0')}>
                            {item.status === 'missing' ? (
                              <>
                                <span className="text-fg-3">∅</span>{' '}
                                <span className="text-accent">→ $62,000</span>
              ...
              <div className="px-4 py-3 border-t border-border-1 bg-bg-1 flex items-center justify-between text-sm text-fg-2">
                <span>Showing 10 of 84 affected rows</span>
                <span className="flex items-center gap-1">
                  <span className="w-2 h-2 rounded-full bg-accent" />
                  <span className="text-accent">Imputed with median ($62,000)</span>
                </span>
              </div>
            </div>
          </div>
